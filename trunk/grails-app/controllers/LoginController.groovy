...
class LoginController {
    ...
    def index = {
        ...
        if (isLoggedIn()) {
            redirect controller:'index', action:index, view:'index'
        }
        else {
...
//	}
    ...
    def denied = {
        if (isLoggedIn() && authenticationTrustResolver.isRememberMe(SCH.context?.authentication)) {
            println("// have cookie but the page is guarded with IS_AUTHENTICATED_FULLY")
    ...
    def authfail = {
        ...
        if (exception) {
            if (exception instanceof DisabledException) {
                msg = "Учетная запись пользователя [$username] отключена."
            }
            else {
                msg = "Пользователь [$username] не существует, либо неверно введен логин/пароль."
            }
        }
//
//		if (isAjax()) {
